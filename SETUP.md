...
```bash
direnv exec . uv run pytest tests/py_lib_runtime -m hermetic -q
direnv exec . uv run pytest tests/py_lib_runtime/integration/test_config_lifecycle.py -q --no-cov
```
...
and reselect `py-lib-runtime (.venv)`.
